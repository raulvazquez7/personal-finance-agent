...
| `POST /merchants/{id}/review` | confirm a merchant: category, subscription, optional rename; sets the default and relabels the merchant's rows that neither the user nor a rule labelled (with an income category, its money-out rows keep theirs); a null `is_subscription` keeps the merchant's flag and each row's own flag |
